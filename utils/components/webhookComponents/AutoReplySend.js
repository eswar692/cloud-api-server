...
const autoReplySend = async (data) => {
  ...
  try {
      ...
      if (autoReply.isActive) {
        const payload = {
          ...
          text: {
            body:
              buttonId === '1'
                ? autoReply.button1.message
                : buttonId === '2'
                ? autoReply.button2.message
                : autoReply.button3.message
          }
        };

        const sendReplyInQuickReply = await axios.post(
          `https://graph.facebook.com/v22.0/${findUser.phoneNumberId}/messages`,
          payload,
          {
            headers: {
    ...
    const lastMessage = await Webhook.find({
      sender: whatsappUserPhoneNumber,
      receiver: apiNumber
    })
      .sort({ timestamp: -1 })
    ...
    const autoReplySent = await sendAutoReplyHelper(
      findUser,
      autoReply,
      message,
      profileName
...
async function sendAutoReplyHelper(
  userApiData,
  autoReply,
  message,
  profileName
  ...
  try {
    const response = await axios.post(
      `https://graph.facebook.com/v22.0/${userApiData.phoneNumberId}/messages`,
      payload,
      {
        headers: {
