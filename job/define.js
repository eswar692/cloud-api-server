...
const agendaDefine = (agenda) => {
  agenda.define('plan-expire', async (job) => {
    try {
      ...
      if (
        user?.prepaidOneMonth?.plan === 'basic Plan' ||
      ...
      const emailOptionsInstance = mailOptions({
        title: 'Plan Expired',
        email: userDetails.email,
        message: {
          text: 'Plan Expired',
          main: 'Please renew your plan',
          sub: 'your plan has been expired'
        },
        otp: null
      });
      ...
      console.error(
        `Error processing plan expiration job: ${error.message}`,
        error
      );
    }
  });
  agenda.define('plan-alert', async (job) => {
    try {
      ...
      const emailOptionsInstance = mailOptions({
        title: 'Your Plan is about to expire in 2 days',
        email: userDetails.email,
        message: {
          text: 'Plan Alert',
          main: 'Please renew your plan',
          sub: 'your plan is about to expire'
        },
        otp: null
      });
      await sendOTP(userDetails.email, emailOptionsInstance);
